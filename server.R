...
function(input, output, session) {
  ...
  dataFilter <- reactive({
    ...
    gameindex <-
      which(((playerdata$G >= games[1]) * (playerdata$G <= games[2])) == 1)
    
    fta <- input$FTA1
    index <- 
      which(((playerdata$FTA >= fta[1]) * (playerdata$FTA <= fta[2])) == 1)
  ...
  observeEvent(input$howToChooseNBA, {
    .generateStatement(session,
                       ...
                       verb = "interacted", 
                       description = "Would you like to select a random player, 
  ...
  player.select <- reactive({
    ### Filter the player data so that it does not choose a player who has no free
    ### throw attempts => no free throw %
  ...
  output$histogramNBA <- renderPlot({
    ...
    ### Calculates the free throw percentages for all the players and puts it into
    ### a variable. Produces NAN's for players that haven't taken any free throws
    ### Doesn't matter for the Histogram though because the Histogram won't
    ### display the NaN's
    for (i in 1:n) {
    ...
    ## Create a ggplot2 object, psuPalette is using here
    ggplot(data = bballdata, aes(x = y)) +
      geom_histogram(
        binwidth = 0.1,
        boundary = 0,
        closed = "left",
  ...
  output$proportion2NBA <- renderPlot({
    ...
    data <- data.frame(
      type = factor(c("Prop","Nu"), levels = c("Prop", "Nu")),
      values = c(phat, h0))
    
    ### Prop = Proportion made (p-hat), Nu = Null value (p_0)
    
    
    ### Create ggplot 
    g <- ggplot(data, aes(x = type, y = values, fill = type)) +
