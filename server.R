...
function(input,output,session){
  ...
  #### make phat ####
  phat <- eventReactive({input$resample
    input$samp.sizeNBA}, {
    h0<-hNBA()
    namedata<-player.select2()
    ftp = namedata$FT/namedata$FTA
  ...
  output$proportion2NBA <-renderPlot({
    ...
    data <- melt(data.frame(p_hat = c(phat), p0 = c(h0),
         hypothesis=c("%made")),
         variable_name="p")

    g <- data %>% ggplot(aes(x=hypothesis, y=value, fill=variable)) + geom_bar(position='dodge', stat='identity') + ylab("Proportion")
    g <- g + theme(axis.text=element_text(size=12),
                   axis.title=element_text(size=14,face="bold"))
    g <- g + if(input$significancebounds){geom_hline(aes(yintercept = lower), color="black")}else{NULL}
    g <- g + if(input$significancebounds){geom_hline(aes(yintercept = upper), color="black")}else{NULL} 
    g + scale_fill_discrete(name="variables",
                            breaks=c("p_hat", "p0"),
